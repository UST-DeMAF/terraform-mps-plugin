...
import ust.tad.terraformmpsplugin.analysis.embeddedDeploymentModels.kubernetes.EmbeddedKubernetesProcessor;
...
public class TransformationService {
    ...
    @Autowired
    private EmbeddedKubernetesProcessor embeddedKubernetesProcessor;

    @Autowired
    private EmbeddedHelmProcessor embeddedHelmProcessor;
     ...
     * transformation is run, using the Gradle build scripts. After that, the resulting EDMM
     * model is imported. Lastly, embedded deployment models are detected and processed.
     *
    ...
    public TechnologyAgnosticDeploymentModel transformInternalToTADM(
        ...
        detectEmbeddedDeploymentModels(taskId, tadm.getTransformationProcessId(),
     ...
     * The first task ensures that MPS is ready to execute the transformation. The second task executes
    ...
    private void runMPSTransformation() throws IOException {
        CommandLine prepareMps =
                CommandLine.parse("./" + mpsLocation + "/gradlew -p " + mpsLocation + " " +
                        "prepareMps");
    ...
    public TechnologyAgnosticDeploymentModel importMPSResult() throws IOException {
        ...
        mapper.addMixIn(
                TechnologyAgnosticDeploymentModel.class,
    ...
    /**
     * Call the processors of embedded deployment models.
     * Currently, supports embedded Helm and Kubernetes.
     *
     ...
     * @param terraformDeploymentModel the input Terraform deployment model from which to extract
     *                                 the embedded deployment models.
     */
    private void detectEmbeddedDeploymentModels(UUID taskId, UUID transformationProcessId,
                                                TerraformDeploymentModel terraformDeploymentModel) {
        embeddedKubernetesProcessor.processEmbeddedKubernetesDeploymentModel(taskId,
                transformationProcessId, terraformDeploymentModel);
        embeddedHelmProcessor.processEmbeddedHelmDeploymentModel(taskId, transformationProcessId,
                terraformDeploymentModel);
    }
