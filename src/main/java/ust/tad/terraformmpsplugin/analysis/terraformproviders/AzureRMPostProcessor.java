...
public class AzureRMPostProcessor {
    ...
    while (componentIterator.hasNext()) {
      ...
      if (componentToTest.getType().getName().equals("azurerm_kubernetes_cluster")) {
        try {
          ...
          throw new PostProcessorFailedException(
              "The AzureRM Post Processor failed " + "with reason: " + e.getMessage());
        }
      }
    }
    return tadm;
  ...
  /**
   * Replaces EDMM components of type AzureRM Kubernetes Cluster with generic components describing
   * physical nodes, operating systems, and container runtimes for each Kubernetes cluster node
   * configured. For each component a corresponding component type is created. New relations between
   * the components are created and existing relations to the aksComponent are redirected to the
   * newly created container runtime component.
   *
   ...
   * @throws InvalidRelationException if a new relation has invalid source or target components
   *     configured
   * @throws InvalidPropertyValueException if a new property is invalid
   */
  private void replaceAKS(TechnologyAgnosticDeploymentModel tadm, Component aksComponent)
    ...
    ComponentType operatingSystemType = AzureRMComponentTypeProvider.createOperatingSystemType();
    ComponentType containerRuntimeType = AzureRMComponentTypeProvider.createContainerRuntimeType();
    ...
    for (int i = 0; i < nodeCount; i++) {
      ...
      if (nodeCount == 1) {
        ...
        physicalNodeComponent.setName(
            aksComponent.getName().concat("_node_").concat(String.valueOf(nodeCount)));
      }
      ...
      Optional<Property> vmSizeProperty =
          aksComponent.getProperties().stream()
              .filter(property -> property.getKey().contains("vm_size"))
              .findFirst();
      if (vmSizeProperty.isPresent()) {
        physicalNodeComponent.setProperties(
            getPropertiesForVMSize(String.valueOf(vmSizeProperty.get().getValue())));
      }
      ...
      if (nodeCount == 1) {
        ...
        operatingSystemComponent.setName(
            aksComponent.getName().concat("_operating-system_").concat(String.valueOf(nodeCount)));
      }
      ...
      if (nodeCount == 1) {
        ...
        containerRuntimeComponent.setName(
            aksComponent.getName().concat("_container-runtime_").concat(String.valueOf(nodeCount)));
      }
      ...
      osOnNodeRelation.setName(
          operatingSystemComponent.getName()
              + "_"
              + hostedOnRelationType.getName()
              + "_"
              + physicalNodeComponent.getName());
      ...
      crOnOSRelation.setName(
          containerRuntimeComponent.getName()
              + "_"
              + hostedOnRelationType.getName()
              + "_"
              + operatingSystemComponent.getName());
